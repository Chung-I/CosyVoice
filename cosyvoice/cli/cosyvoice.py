...
class CosyVoice:
    ...
    def inference_zero_shot(self, tts_text, prompt_text, prompt_speech_16k, llm_request, stream=False, speed=1.0, text_frontend=True):
        ...
        for i in tqdm(self.frontend.text_normalize(tts_text, split=True, text_frontend=text_frontend)):
            ...
            start_time = time.time()
            logging.info('synthesis text {}'.format(i))
            for model_output in self.model.tts(**model_input, stream=stream, speed=speed, llm_request=llm_request):
                speech_len = model_output['tts_speech'].shape[1] / self.sample_rate
                logging.info('yield speech len {}, rtf {}'.format(speech_len, (time.time() - start_time) / speech_len))
                yield model_output
                start_time = time.time()
